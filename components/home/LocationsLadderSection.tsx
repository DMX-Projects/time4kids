...
import type { CityTile } from '@/lib/public-franchise-cities';

const slatColors = ['bg-[#38c1ed]', 'bg-[#dbbe3e]', 'bg-[#6cc3ba]'];

type Props = {
    tiles: CityTile[];
    /** true = loaded from franchise API; false = static fallback list */
    fromDatabase?: boolean;
};

/** Server-rendered “Our Presence” city pills (no client JS required). */
export default function LocationsLadderSection({ tiles, fromDatabase = false }: Props) {
    return (
        <section className="locations-ladder-section section-gap bg-gradient-to-b from-[#ffffff] to-[#dbf5ff]">
            <div className="container relative z-10 mx-auto w-full max-w-[1600px] px-4">
                <div className="mx-auto mb-10 max-w-4xl text-center">
                    ...
                    <h2 className="font-display text-4xl font-black leading-tight tracking-[-0.02em] text-[#253247] md:text-6xl">
                        Our{' '}
                    ...
                    <p className="mx-auto mt-2 max-w-2xl text-sm leading-7 text-slate-500">
                        {fromDatabase
                            ? `${tiles.length} cities from active centres (franchise table).`
                            : `${tiles.length} cities (fallback list — start Django API to sync from database).`}
                    </p>
