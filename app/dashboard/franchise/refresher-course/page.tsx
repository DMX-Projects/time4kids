...
export default function RefresherCoursePage() {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                const data = await authFetch<any>("/documents/franchise/documents/category/REFRESHER_COURSE/");
                setDocs(Array.isArray(data) ? data : []);
            } catch {
                showToast("Could not load Refresher Course documents. Please try again.", "error");
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="space-y-5">
            <div className="space-y-1">
                <h1 className="text-2xl font-semibold text-[#111827]">Refresher Course</h1>
                <p className="text-sm text-[#374151]">Facilitator refresher guides and assessments.</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-[#fffaf0] border border-[#E5E7EB] rounded-2xl p-5 text-sm text-[#374151]">
                    No Refresher Course documents found for your centre yet.
                </div>
            ) : (
