...
export default function InfrastructureManualPage() {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                const data = await authFetch<any>("/documents/franchise/documents/category/INFRASTRUCTURE_MANUAL/");
                setDocs(Array.isArray(data) ? data : []);
            } catch {
                showToast("Could not load Infrastructure documents. Please try again.", "error");
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="space-y-5">
            <div className="space-y-1">
                <h1 className="text-2xl font-semibold text-[#111827]">Infrastructure Manual & Purchase List</h1>
                <p className="text-sm text-[#374151]">Approved vendors, fixtures, and procurement lists.</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-[#fffaf0] border border-[#E5E7EB] rounded-2xl p-5 text-sm text-[#374151]">
                    No Infrastructure documents found for your centre yet.
                </div>
            ) : (
