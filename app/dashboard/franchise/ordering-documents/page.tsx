...
export default function OrderingDocumentsPage() {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                const data = await authFetch<any>("/documents/franchise/documents/category/ORDERING_DOCUMENTS/");
                setDocs(Array.isArray(data) ? data : []);
            } catch {
                showToast("Could not load Ordering documents. Please try again.", "error");
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="space-y-5">
            <div className="space-y-1">
                <h1 className="text-2xl font-semibold text-[#111827]">Ordering Documents (IK & SM)</h1>
                <p className="text-sm text-[#374151]">Ordering flows and payment handoffs with available templates.</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-[#fffaf0] border border-[#E5E7EB] rounded-2xl p-5 text-sm text-[#374151]">
                    No Ordering documents found for your centre yet.
                </div>
            ) : (
