...
export default function AksharabhyasamSupportSheetsPage() {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                const data = await authFetch<any>("/documents/franchise/documents/category/AKSHARABHYASAM_SUPPORT_SHEETS/");
                setDocs(Array.isArray(data) ? data : []);
            } catch {
                showToast("Could not load Aksharabhyasam Support Sheets. Please try again.", "error");
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="space-y-5">
            <div className="space-y-1">
                <h1 className="text-2xl font-semibold text-[#111827]">Aksharabhyasam Support Sheets</h1>
                <p className="text-sm text-[#374151]">Support worksheets and practice trackers.</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-[#fffaf0] border border-[#E5E7EB] rounded-2xl p-5 text-sm text-[#374151]">
                    No Aksharabhyasam support sheets found for your centre yet.
                </div>
            ) : (
