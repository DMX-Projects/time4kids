...
export default function AchievementsPage() {
    const { achievements, achievementsLoading, selectedStudent, hasMultipleChildren } = useParentData();
    ...
    return (
        <div className="space-y-6">
            <Section
                ...
                description={
                    hasMultipleChildren && selectedStudent
                        ? `Milestones for ${selectedStudent.name} and centre-wide awards. Switch child from the header.`
                        : "Milestones, awards, and recognitions shared by your centre. View-only for parents."
                }
                icon={<Star className="w-5 h-5 text-orange-600" />}
            >
                <p className="text-sm text-orange-700">
                    These achievements are published by your centre. If something looks incorrect, please contact them.
                </p>
            </Section>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
                {visibleAchievements.map((ach) => (
                    <div key={ach.id} className="bg-white border border-orange-100 rounded-xl p-4 shadow-sm">
                        <div className="flex items-center justify-between mb-1 gap-2 flex-wrap">
                            ...
                            <span className="text-xs px-2 py-1 bg-orange-50 text-orange-700 rounded-full border border-orange-100 shrink-0">
                                {ach.date || "—"}
                            </span>
                        </div>
                        {ach.scope === "centre" && (
                ...
                {!achievementsLoading && visibleAchievements.length === 0 && (
                    <p className="text-sm text-orange-700">
                        {achievements.length > 0 && selectedStudent
                            ? `No achievements for ${selectedStudent.name} yet.`
