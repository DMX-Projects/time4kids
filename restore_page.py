                                            ...
                                            <tr key={enq.id} className=\"hover:bg-slate-50/50 transition-colors group\">
                                                ...
                                                <td className=\"px-6 py-4 align-top\">
                                                    <p className=\"text-slate-600 text-xs leading-relaxed line-clamp-3\" title={parseEnquiryMessage(enq.message, enq.type).userMessage || parseEnquiryMessage(enq.message, enq.type).note}>
                                                        {parseEnquiryMessage(enq.message, enq.type).userMessage || parseEnquiryMessage(enq.message, enq.type).note || \"-\"}
                                                    </p>
                                                </td>
                                ...
                                <>
                                    ...
                                    <div className={`${theme.bg} rounded-xl p-4 grid grid-cols-2 gap-y-4 gap-x-6 border ${theme.border}`}>
                                        ...
                                        <div className=\"space-y-1\">
                                            ...
                                            <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium border ${getStatusColor(selectedEnquiry.status)}`}>
                                                {selectedEnquiry.status === 'new' ? 'Pending' :
                                                 selectedEnquiry.status === 'contacted' ? 'Called/Contacted' :
                                                 selectedEnquiry.status === 'dropped' ? 'Dropped/Not Interested' :
                                    ...
                                    {notesHistory.length > 0 && (
                                        <div className=\"space-y-3\">
                                            ...
                                            <div className=\"bg-slate-50 rounded-lg p-3 space-y-3 max-h-40 overflow-y-auto border border-slate-100\">
                                                {notesHistory.map((n, i) => (
                                                    <div key={i} className=\"border-b border-slate-100 pb-2 last:border-0 last:pb-0\">
                                                        ...
                                                        <p className=\"text-slate-400 text-xs mt-1\">
                                                            {new Date(n.created_at).toLocaleString()} by {n.created_by_name}
                                                        </p>
                                                    </div>
                                                ))}
                                            </div>
...
with open("c:\\Users\\Admin1\\Desktop\\time4kids website\\time4kids\\restore_page.py", "w", encoding="utf-8") as f:
    f.write(content)
